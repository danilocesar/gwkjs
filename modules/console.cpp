...
static void
gwkjs_console_error_reporter(JSContext *cx, const char *message, JSErrorReport *report)
{
    int i, j, k, n;
    char *prefix, *tmp;
    const char *ctmp;
...
#ifdef HAVE_LIBREADLINE
static JSBool
gwkjs_console_readline(JSContext *cx, char **bufp, FILE *file, const char *prompt)
{
    char *line;
...
JSBool
gwkjs_console_interact(JSContext *context,
                     unsigned   argc,
                     jsval     *vp)
{
    ...
    JS_SetErrorReporter(context, gwkjs_console_error_reporter);
    ...
    do {
        ...
        if (str) {
            char *display_str;
            display_str = gwkjs_value_debug_string(context, result);
            if (display_str != NULL) {
                g_fprintf(stdout, "%s\n", display_str);
                g_free(display_str);
            }
...
JSBool
gwkjs_define_console_stuff(JSContext  *context,
                         JSObject  **module_out)
{
    JSObject *module;
    ...
    if (!JS_DefineFunction(context, module,
                           "interact",
                           (JSNative) gwkjs_console_interact,
